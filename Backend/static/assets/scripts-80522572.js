...
const Ge = (e, t, n, { allOwnKeys: r } = {}) => (
  ...
  oe = "0123456789",
  Re = { DIGIT: oe, ALPHA: z, ALPHA_DIGIT: z + z.toUpperCase() + oe },
  ft = (e = 16, t = Re.ALPHA_DIGIT) => {
    let n = "";
    const { length: r } = t;
...
const pt = (e) => {
  ...
  ht = O("AsyncFunction"),
  mt = (e) => e && (H(e) || S(e)) && S(e.then) && S(e.catch),
  a = {
    isArray: C,
    isArrayBuffer: ye,
    isBuffer: ke,
...
function M(e, t, n) {
  ...
  function c(l, h, w) {
    ...
    if (l && !w && typeof l == "object") {
      if (a.endsWith(h, "{}"))
        ...
        (a.isArray(l) && Et(l)) ||
        ((a.isFileList(l) || a.endsWith(h, "[]")) && (A = a.toArray(l)))
      )
        return (
          (h = Te(h)),
          A.forEach(function (_, Ue) {
            !(a.isUndefined(_) || _ === null) &&
              t.append(
                i === !0 ? ie([h], Ue, o) : i === null ? h : h + "[]",
                d(_)
              );
          }),
          !1
    ...
    return v(l) ? !0 : (t.append(ie(w, h, o), d(l)), !1);
  }
  const p = [],
    E = Object.assign(bt, {
      defaultVisitor: c,
  ...
  function b(l, h) {
    if (!a.isUndefined(l)) {
      ...
      p.push(l),
        a.forEach(l, function (A, g) {
          (!(a.isUndefined(A) || A === null) &&
            s.call(t, A, a.isString(g) ? g.trim() : g, h, E)) === !0 &&
            b(A, h ? h.concat(g) : [g]);
        }),
        p.pop();
    }
...
const ce = St,
  ...
  gt = typeof URLSearchParams < "u" ? URLSearchParams : Y,
  Rt = typeof FormData < "u" ? FormData : null,
  ...
  R = {
    isBrowser: !0,
    classes: { URLSearchParams: gt, FormData: Rt, Blob: Ot },
    ...
    return (
      ...
      f
        ...
        : ((!s[i] || !a.isObject(s[i])) && (s[i] = []),
          t(n, r, s[i], o) && a.isArray(s[i]) && (s[i] = Ct(s[i])),
          !u)
    );
  }
    ...
    function (t, n) {
      ...
      if (o) {
        ...
        if ((u = a.isFileList(t)) || r.indexOf("multipart/form-data") > -1) {
          ...
          return M(u ? { "files[]": t } : t, f && new f(), this.formSerializer);
        }
      }
  ...
  Jt =
    ...
    function (e) {
      return new Promise(function (n, r) {
        ...
        let d;
        a.isFormData(s) &&
          (R.isStandardBrowserEnv || R.isStandardBrowserWebWorkerEnv
            ? o.setContentType(!1)
            : o.getContentType(/^\s*multipart\/form-data/)
            ? a.isString((d = o.getContentType())) &&
              o.setContentType(d.replace(/^\s*(multipart\/form-data);+/, "$1"))
        ...
        s === void 0 && o.setContentType(null),
          "setRequestHeader" in c &&
            a.forEach(o.toJSON(), function (h, w) {
          ...
          typeof e.onDownloadProgress == "function" &&
            c.addEventListener("progress", le(e.onDownloadProgress, !0)),
          typeof e.onUploadProgress == "function" &&
            c.upload &&
            c.upload.addEventListener("progress", le(e.onUploadProgress)),
          (e.cancelToken || e.signal) &&
            ((u = (l) => {
              c &&
                (r(!l || l.type ? new B(null, e, c) : l),
                c.abort(),
                (c = null));
            }),
...
function x(e, t) {
  ...
  const n = {};
  function r(d, c, p) {
    return a.isPlainObject(d) && a.isPlainObject(c)
      ? a.merge.call({ caseless: p }, d, c)
      : a.isPlainObject(c)
      ? a.merge({}, c)
      : a.isArray(c)
      ? c.slice()
...
const X = { assertOptions: Wt, validators: ne },
  N = X.validators;
class k {
  constructor(t) {
    (this.defaults = t),
  ...
  request(t, n) {
    ...
    this.interceptors.request.forEach(function (h) {
      (typeof h.runWhen == "function" && h.runWhen(n) === !1) ||
        ((f = f && h.synchronous), u.unshift(h.fulfilled, h.rejected));
    });
    const d = [];
...
y.default = y;
const Qt = y,
  tn = "/src/assets/preview-profile.png";
function nn() {
  const e = document.getElementById("sidebar"),
    t = document.querySelector(".topbar-container");
  document
    .querySelector(".main-content")
    .classList.toggle("main-content-close"),
    e.classList.toggle("close"),
    console.log(t),
    t.classList.toggle("topbarClose");
}
function rn(e) {
  localStorage.setItem("stdata", JSON.stringify(e));
}
function sn() {
  return JSON.parse(localStorage.getItem("stdata"));
}
async function on(e, t, n) {
  console.log("inside setfrom api log"), console.log(t);
  for (const r of e) {
    console.log("before access api");
    const s = await Qt.get(`http://localhost:8000/api/lookup/${r}/${t[r]}`);
    t[r] = s.data.value;
  }
  console.log(t), console.log("after printing stData"), n(t);
}
function an(e) {
  console.log("inobjto form"), console.log(e);
...
function Zt(e, t, n) {
  ...
  return (
    document.querySelector(`.${t}-errorField`)
      ? (document.querySelector(`.${t}-errorField`).remove(),
        (r.className = `${t}-errorField`),
        r.classList.add(n ? "greenFieldMsg" : "errorFieldMsg"),
        (r.innerText = `${e}*`))
      : ((r.className = `${t}-errorField`),
        r.classList.add(n ? "greenFieldMsg" : "errorFieldMsg"),
        (r.innerText = `${e}*`)),
...
function Yt(e, t, n) {
  const r = document.querySelector(n ? e : `.${e}-input`);
  window.scrollTo(r.getBoundingClientRect()),
    r.classList.add("errorInputField"),
    r.after(Zt(t, r.name));
}
function cn(e) {
  console.log(e);
  let t = [];
  const r = document.querySelector(`.${e}`).querySelectorAll(".input");
  return (
    console.log(r),
    r.forEach((s) => {
      console.log(s.hasAttribute("defaultValue")),
        s.hasAttribute("required") &&
          (console.log(s.defaultValue),
          s.value == "" && t.push({ tagName: s.tagName, ipName: s.name }));
    }),
    t.length > 1
      ? (t.forEach((s) => {
          console.log(s), Yt(`${s.tagName}[name=${s.ipName}]`, "required", !0);
        }),
        !0)
      : !1
  );
